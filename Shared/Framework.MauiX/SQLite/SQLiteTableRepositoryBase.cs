...
{
    public static TableQuery<TSQLiteItem> Sort<TSQLiteItem, U>(this TableQuery<TSQLiteItem> tableQuery, Expression<Func<TSQLiteItem, U>> orderByExpression, QueryOrderDirections direction)
    {
        if (direction == QueryOrderDirections.Ascending)
            return tableQuery.OrderBy(orderByExpression);
        return tableQuery.OrderByDescending(orderByExpression);
    }
    public static TableQuery<TSQLiteItem> ThenSort<TSQLiteItem, U>(this TableQuery<TSQLiteItem> tableQuery, Expression<Func<TSQLiteItem, U>> orderByExpression, QueryOrderDirections direction)
    {
        if (direction == QueryOrderDirections.Ascending)
...
{
    public SQLiteTableRepositoryBase(SQLiteService sqLiteService)
        : base(sqLiteService)
    {
    }
    ...
    public virtual async Task<List<TItem>> Search(
        TAdvancedQuery query, ObservableQueryOrderBySetting queryOrderBySetting)
    {
        return await Search(query, queryOrderBySetting.Direction, (Func<TableQuery<TItem>, QueryOrderDirections, TableQuery<TItem>>)queryOrderBySetting.SortFunc);
    }

    protected async Task<List<TItem>> Search(
        TAdvancedQuery query
        , QueryOrderDirections direction
        , Func<TableQuery<TItem>, QueryOrderDirections, TableQuery<TItem>> sortFunction)
    {
        var predicate = GetSQLiteTableQueryPredicateByAdvancedQuery(query);
    ...
    where TItem : new()
{
    ...
    protected Task<List<TItem>> GetItemRangeFromTableAsync<U>(Expression<Func<TItem, bool>> predicate, int skip, int take, Expression<Func<TItem, U>> orderExp, QueryOrderDirections orderDirections = QueryOrderDirections.Ascending)
    {
        try
        {
            ...
            if (orderDirections == QueryOrderDirections.Ascending)
            {
