...
import { createComposite } from 'src/slices/SalesOrderDetailSlice';
...
export default function CreateWizardPartial(props: WizardPartialProps<ISalesOrderDetailCompositeModel>): JSX.Element {
    ...
    const onSubmit = () => {
        ...
        } as ISalesOrderDetailCompositeModel;
    ...
    const renderWizardStep = (step: WizardStepProps, index: number, renderWizardButtonGroup: (isFirstStep: boolean, isLastStep: boolean, isStepOptional: boolean, disableNextButton: () => boolean) => JSX.Element,) => {
        return (
            <>
                ...
                {step.id === "__Master__" && <SalesOrderDetailCreatePartial {...getCRUDItemPartialViewPropsWizard<ISalesOrderDetailDataModel>(ViewItemTemplates.Create, orientation, (data: ISalesOrderDetailDataModel) => { set__Master__(data); })} item={__Master__} isFirstStep={isFirstStep(index)} isLastStep={isLastStep(index)} isStepOptional={step.optional} renderWizardButtonGroup={renderWizardButtonGroup} />}
