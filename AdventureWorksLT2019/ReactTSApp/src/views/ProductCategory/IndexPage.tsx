...
import { productCategorySelectors, search, bulkDelete } from 'src/slices/ProductCategorySlice';
import { defaultIProductCategoryAdvancedQuery, getProductCategoryQueryOrderBySettings, IProductCategoryAdvancedQuery } from 'src/dataModels/IProductCategoryQueries';
...
export default function IndexPage() {
    ...
    const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
        if (event.target.checked) {
            const newSelected = listItems.map((n) => n.productCategoryID);
    ...
    const handleRefresh = () => {
        if (!isLoading) {
            setAdvancedQuery(defaultIProductCategoryAdvancedQuery());
            setSelected([]);
            setIsLoading(true);
            dispatch(search(advancedQuery)).finally(() => { setIsLoading(false); });
        }
    ...
    const handleDeleteSelected = () => {
		/*
        const identifiers = selected.map((id) => { return { productCategoryID: id } });
    ...
    const submitAdvancedSearch = (query: IProductCategoryAdvancedQuery) => {
    ...
    const handleSelectItemClick = (item: IProductCategoryDataModel) => {
        const selectedIndex = selected.indexOf(item.productCategoryID);
        let newSelected: readonly number[] = [];

        if (selectedIndex === -1) {
            newSelected = newSelected.concat(selected, item.productCategoryID);
            ...
            >
                <Box>
                    <Checkbox
                        ...
                        inputProps={{
                            'aria-label': 'select All ProductCategory',
                        }} />
                    ...
                    <Box sx={{ p: 0, display: 'flex', alignItems: 'center' }}>
                        ...
                        >
                            <OutlinedInput
                                ...
                                placeholder={t("Search") + ' ' + t("ProductCategory")}
