...
import { createComposite } from 'src/slices/ProductSlice';
...
const wizardSteps = [
    ...
    {
        id: 'SalesOrderDetails_Via_ProductID',
        wizardStepOptions: WizardStepOptions.Editor,
        label: 'SalesOrderDetails_Via_ProductID',
        description: 'SalesOrderDetails_Via_ProductID',
        icon: <Map fontSize='small' />,
        optional: true,
    },
    {
...
export default function CreateWizardPartial(props: WizardPartialProps<IProductCompositeModel>): JSX.Element {
    ...
    const handleReset = () => {
        ...
        setSalesOrderDetails_Via_ProductID(props.data.salesOrderDetails_Via_ProductID);
    };
    const handleSkip = () => {
    ...
    const [salesOrderDetails_Via_ProductID, setSalesOrderDetails_Via_ProductID] = React.useState(props.data.salesOrderDetails_Via_ProductID);
    ...
    const renderWizardStep = (step: WizardStepProps, index: number, renderWizardButtonGroup: (isFirstStep: boolean, isLastStep: boolean, isStepOptional: boolean, disableNextButton: () => boolean, submitRef: React.MutableRefObject<any>) => JSX.Element,) => {
        return (
            <Box sx={{height: '70vh'}}>
                ...
                {step.id === "SalesOrderDetails_Via_ProductID" && (renderSalesOrderDetails_Via_ProductID_FirstInList(step, index, renderWizardButtonGroup))}
        ...
        return (
            <ProductCreatePartial
                {...getCRUDItemPartialViewPropsWizard<IProductDataModel>(
                    ViewItemTemplates.Create, orientation, (data: IProductDataModel) => { set__Master__(data); })} item={__Master__}
    ...
    const renderSalesOrderDetails_Via_ProductID_FirstInList = (step: WizardStepProps, index: number, renderWizardButtonGroup: (isFirstStep: boolean, isLastStep: boolean, isStepOptional: boolean, disableNextButton: () => boolean, submitRef: React.MutableRefObject<any>) => JSX.Element) => {
        return (
            <SalesOrderDetailCreatePartial
                {...getCRUDItemPartialViewPropsWizard<ISalesOrderDetailDataModel>(
                    ViewItemTemplates.Create, orientation, (data: ISalesOrderDetailDataModel) => { setSalesOrderDetails_Via_ProductID([data]); })} item={salesOrderDetails_Via_ProductID[0]}
                isFirstStep={isFirstStep(index)}
                isLastStep={isLastStep(index)}
                isStepOptional={step.optional}
                renderWizardButtonGroup={renderWizardButtonGroup} />
