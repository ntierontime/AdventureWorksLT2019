    ...
    const renderWizardStep = (step: WizardStepProps, index: number, renderWizardButtonGroup: (isFirstStep: boolean, isLastStep: boolean, isStepOptional: boolean, disableNextButton: () => boolean) => JSX.Element,) => {
        return (
            <>
                ...
                {step.id === "__Master__" && <ProductDescriptionCreatePartial {...getCRUDItemPartialViewPropsWizard<IProductDescriptionDataModel>(ViewItemTemplates.Create, orientation, (data: IProductDescriptionDataModel) => { set__Master__(data); })} item={__Master__} isFirstStep={isFirstStep(index)} isLastStep={isLastStep(index)} isStepOptional={step.optional} renderWizardButtonGroup={renderWizardButtonGroup} />}
