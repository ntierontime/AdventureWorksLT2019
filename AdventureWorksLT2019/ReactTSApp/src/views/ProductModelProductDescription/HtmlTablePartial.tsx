...
export default function HtmlTablePartial(props: ListPartialViewProps<IProductModelProductDescriptionDataModel, IProductModelProductDescriptionIdentifier>): JSX.Element {
    ...
    const orderedListItems = useMemo(() => !!listItems ? stableSort(listItems, getComparator(order, orderBy)) as IProductModelProductDescriptionDataModel[] : [], [listItems, order, orderBy]);
    ...
    const handleClientSideRequestSort = (
        ...
        property: keyof IProductModelProductDescriptionDataModel,
    ) => {
    ...
    return (
        <>
                ...
                >
                    ...
                    rows.slice().sort(getComparator(order, orderBy)) */}
                        {orderedListItems
                            ...
                            .map((row, index) => {
								const key = getRouteParamsOfIProductModelProductDescriptionIdentifier(row);
                                const isItemSelected = isSelected(getIProductModelProductDescriptionIdentifier(row));
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <TableCell
                                            ...
                                            padding="none"
                                        >
                                            <Link href={"/productModel/Details/" + row.productModelID}>{row.productModel_Name}</Link>
                                        </TableCell>
                                        <TableCell align='right'><Link href={"/productDescription/Details/" + row.productDescriptionID}>{row.productDescription_Name}</Link></TableCell>
            ...
            >
                ...
                <IconButton aria-label="delete" color="primary" onClick={() => { navigate("/productModelProductDescription/delete/" + getRouteParamsOfIProductModelProductDescriptionIdentifier(currentItemOnDialog)) }}>
                    <DeleteIcon />
                </IconButton>
                <IconButton aria-label="details" color="primary" onClick={() => { navigate("/productModelProductDescription/details/" + getRouteParamsOfIProductModelProductDescriptionIdentifier(currentItemOnDialog)) }}>
                    <BusinessCenterIcon />
                </IconButton>
                ...
                <IconButton aria-label="dashboard" color="primary" onClick={() => { navigate("/productModelProductDescription/dashboard/" + getRouteParamsOfIProductModelProductDescriptionIdentifier(currentItemOnDialog)) }}>
                    <AccountTreeIcon />
                </IconButton>
            </Popover>
