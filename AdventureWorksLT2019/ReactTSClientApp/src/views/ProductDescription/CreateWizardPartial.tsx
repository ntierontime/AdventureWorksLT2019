...
import { createComposite } from 'src/slices/ProductDescriptionSlice';
...
import { default as ProductDescriptionCreatePartial } from '../ProductDescription/CreatePartial';
...
const wizardSteps = [
    ...
    {
        id: 'ProductModelProductDescriptions_Via_ProductDescriptionID',
        wizardStepOptions: WizardStepOptions.Editor,
        label: 'ProductModelProductDescriptions_Via_ProductDescriptionID',
        description: 'ProductModelProductDescriptions_Via_ProductDescriptionID',
        icon: <Map fontSize='small' />,
        optional: true,
    },
    {
...
export default function CreateWizardPartial(props: WizardPartialProps<IProductDescriptionCompositeModel>): JSX.Element {
    ...
    const handleReset = () => {
        ...
        setProductModelProductDescriptions_Via_ProductDescriptionID(props.data.productModelProductDescriptions_Via_ProductDescriptionID);
    };
    const handleSkip = () => {
    ...
    const onSubmit = () => {
        ...
        } as IProductDescriptionCompositeModel;
    ...
    const [productModelProductDescriptions_Via_ProductDescriptionID, setProductModelProductDescriptions_Via_ProductDescriptionID] = React.useState(props.data.productModelProductDescriptions_Via_ProductDescriptionID);
    ...
    const renderWizardStep = (step: WizardStepProps, index: number, renderWizardButtonGroup: (isFirstStep: boolean, isLastStep: boolean, isStepOptional: boolean, disableNextButton: () => boolean, submitRef: React.MutableRefObject<any>) => JSX.Element,) => {
        return (
            <Box sx={{height: '70vh'}}>
                ...
                {step.id === "ProductModelProductDescriptions_Via_ProductDescriptionID" && (renderProductModelProductDescriptions_Via_ProductDescriptionID_FirstInList(step, index, renderWizardButtonGroup))}
    ...
    const render__Master___Single = (step: WizardStepProps, index: number, renderWizardButtonGroup: (isFirstStep: boolean, isLastStep: boolean, isStepOptional: boolean, disableNextButton: () => boolean, submitRef: React.MutableRefObject<any>) => JSX.Element) => {
        return (
            <ProductDescriptionCreatePartial
                {...getCRUDItemPartialViewPropsWizard<IProductDescriptionDataModel>(
                    ViewItemTemplates.Create, orientation, (data: IProductDescriptionDataModel) => { set__Master__(data); })} item={__Master__}
    ...
    const renderProductModelProductDescriptions_Via_ProductDescriptionID_FirstInList = (step: WizardStepProps, index: number, renderWizardButtonGroup: (isFirstStep: boolean, isLastStep: boolean, isStepOptional: boolean, disableNextButton: () => boolean, submitRef: React.MutableRefObject<any>) => JSX.Element) => {
        return (
            <ProductModelProductDescriptionCreatePartial
                {...getCRUDItemPartialViewPropsWizard<IProductModelProductDescriptionDataModel>(
                    ViewItemTemplates.Create, orientation, (data: IProductModelProductDescriptionDataModel) => { setProductModelProductDescriptions_Via_ProductDescriptionID([data]); })} item={productModelProductDescriptions_Via_ProductDescriptionID[0]}
                isFirstStep={isFirstStep(index)}
                isLastStep={isLastStep(index)}
                isStepOptional={step.optional}
                renderWizardButtonGroup={renderWizardButtonGroup} />
