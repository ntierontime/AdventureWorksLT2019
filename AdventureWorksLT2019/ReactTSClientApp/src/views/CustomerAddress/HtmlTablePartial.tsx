...
import { ICustomerAddressIdentifier, getICustomerAddressIdentifier, getRouteParamsOfICustomerAddressIdentifier } from 'src/dataModels/ICustomerAddressQueries';
...
export default function HtmlTablePartial(props: ListPartialViewProps<ICustomerAddressDataModel, ICustomerAddressIdentifier>): JSX.Element {
    ...
    const orderedListItems = useMemo(() => !!listItems ? stableSort(listItems, getComparator(order, orderBy)) as ICustomerAddressDataModel[] : [], [listItems, order, orderBy]);
                ...
                >
                    ...
                    rows.slice().sort(getComparator(order, orderBy)) */}
                        {orderedListItems
                            ...
                            .map((row, index) => {
								const key = getRouteParamsOfICustomerAddressIdentifier(row);
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <TableCell
                                            ...
                                            padding="none"
                                        >
                                            <Link href={"/customer/Details/" + row.customerID}>{row.customer_Name}</Link>
                                        </TableCell>
                                        <TableCell align='right'><Link href={"/address/Details/" + row.addressID}>{row.address_Name}</Link></TableCell>
