...
import { IProductModelDataModel, productModelFormValidation } from 'src/dataModels/IProductModelDataModel';


export default function EditPartial(props: ItemCardProps<IProductModelDataModel>): JSX.Element {
    ...
    const methods = useForm({
        ...
        defaultValues: item,
        resolver: yupResolver(productModelFormValidation)
    });
    ...
    const renderMainButtonGroup = () => {
        ...
        if (mainButtonType === ButtonTypes.IconAndText) {
            return (
                <EditItemActionIconAndTextButtonGroup previousAction={previousAction} nextAction={nextAction} doneAction={doneAction} submitted={submitted} submitting={submitting} isValid={isValid} extraButtonGroups={extraButtonGroups} formId="ProductModelEditForm" />
            );
        }
        ...
        return (
            <EditItemActionIconButtonGroup previousAction={previousAction} nextAction={nextAction} doneAction={doneAction} submitted={submitted} submitting={submitting} isValid={isValid} extraButtonGroups={extraButtonGroups} formId="ProductModelEditForm" />
        );
    }

    return (
        <Card component="form" noValidate onSubmit={(event) => { event.preventDefault(); event.stopPropagation(); handleSubmit(onSubmit)(event); }}  id="ProductModelEditForm" sx={{ ...card100PercentHeighFlex }} >
