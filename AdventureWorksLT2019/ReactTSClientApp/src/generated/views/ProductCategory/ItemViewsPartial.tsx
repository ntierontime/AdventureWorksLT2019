...
import { defaultProductCategory, IProductCategoryDataModel } from 'src/dataModels/IProductCategoryDataModel';
import { getIProductCategoryIdentifier, getRouteParamsOfIProductCategoryIdentifier } from 'src/dataModels/IProductCategoryQueries';

import { post, put } from 'src/generated/slices/ProductCategorySlice';
...
export default function ItemViewsPartial(props: ItemViewsPartialProps<IProductCategoryDataModel>): JSX.Element {
    ...
    const onSubmit_WhenCreate_DispatchAsyncThunk = (data: IProductCategoryDataModel, index: number) => {
        setSubmitting(true);
    ...
    const onSubmit_WhenEdit_DispatchAsyncThunk = (data: IProductCategoryDataModel, index: number) => {
        setSubmitting(true);
        dispatch(put({ identifier: getIProductCategoryIdentifier(data), data: { ...data } }))
            .then((result: any) => {
                if (!!result && !!result.meta && result.meta.requestStatus === 'fulfilled') { // success
                    setSubmitMessage(t('SuccessfullySaved'));
                    setSubmitted(true);
    ...
    return (
        <>
            ...
            {viewItemTemplate === ViewItemTemplates.Create &&
                <CreatePartial {...defaultItemCardProps} doneAction={doneAction} item={defaultProductCategory()}
