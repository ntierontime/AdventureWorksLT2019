...
import { Checkbox, Chip, FormControlLabel, FormLabel, IconButton, Link, Pagination, Popover, Stack, Switch, Table, TableBody, TableCell, TableContainer, TableRow } from '@mui/material';
                            ...
                            .map((row, index) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <TableCell align='center'>{t(i18nFormats.number.format, { val: row.revisionNumber })}</TableCell>
                                        <TableCell align='center'>{t(i18nFormats.dateTime.format, { val: new Date(row.orderDate), formatParams: { val: i18nFormats.dateTime.dateTimeShort, } })}</TableCell>
                                        <TableCell align='center'>{t(i18nFormats.dateTime.format, { val: new Date(row.dueDate), formatParams: { val: i18nFormats.dateTime.dateTimeShort, } })}</TableCell>
                                        <TableCell align='center'>{t(i18nFormats.dateTime.format, { val: new Date(row.shipDate), formatParams: { val: i18nFormats.dateTime.dateTimeShort, } })}</TableCell>
                                        <TableCell align='center'>{t(i18nFormats.number.format, { val: row.status })}</TableCell>
                                        <TableCell align='center'><Checkbox checked={row.onlineOrderFlag} readOnly /></TableCell>
                                        ...
                                        <TableCell align='center'><Link href={"/customer/Details/" + row.customerID}>{row.customer_Name}</Link></TableCell>
                                        <TableCell align='center'><Link href={"/address/Details/" + row.shipToAddressID}>{row.shipTo_Name}</Link></TableCell>
                                        <TableCell align='center'><Link href={"/address/Details/" + row.billToAddressID}>{row.billTo_Name}</Link></TableCell>
                                        <TableCell align='center'>{row.shipMethod}</TableCell>
                                        <TableCell align='center'>{row.creditCardApprovalCode}</TableCell>
                                        <TableCell align='center'>{t(i18nFormats.double.format, { val: row.subTotal })}</TableCell>
                                        <TableCell align='center'>{t(i18nFormats.double.format, { val: row.taxAmt })}</TableCell>
                                        <TableCell align='center'>{t(i18nFormats.double.format, { val: row.freight })}</TableCell>
                                        <TableCell align='center'>{t(i18nFormats.double.format, { val: row.totalDue })}</TableCell>
                                        <TableCell align='center'>{row.comment}</TableCell>
                                        <TableCell align='center'>{row.rowguid}</TableCell>
                                        <TableCell align='center'>{t(i18nFormats.dateTime.format, { val: new Date(row.modifiedDate), formatParams: { val: i18nFormats.dateTime.dateTimeShort, } })}</TableCell>
