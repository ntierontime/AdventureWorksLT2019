        ...
        : IAddressRepository
    {
            ...
            AddressAdvancedQuery query)
        {
            try
            {
                ...
                return new ListResponse<AddressDataModel[]>
                {
            ...
            {
                return await Task<ListResponse<AddressDataModel[]>>.FromResult(new ListResponse<AddressDataModel[]>
                {
        ...
        public async Task<Response<AddressDataModel>> Update(AddressIdentifier id, AddressDataModel input, string[]? toUpdatePropertyList = null)
        {
            if (input == null)
        ...
        private static void CopyUpdateValues(AddressDataModel? input, string[]? toUpdatePropertyList, Address existing)
        {
            if (input == null)
                return;

            // 1. This Table - Address
            ...
            {
                if(toUpdatePropertyList.Contains(nameof(AddressDataModel.AddressLine1)))
                    existing.AddressLine1 = input.AddressLine1;
                if(toUpdatePropertyList.Contains(nameof(AddressDataModel.AddressLine2)))
                    existing.AddressLine2 = input.AddressLine2;
                if(toUpdatePropertyList.Contains(nameof(AddressDataModel.City)))
                    existing.City = input.City;
                if(toUpdatePropertyList.Contains(nameof(AddressDataModel.StateProvince)))
                    existing.StateProvince = input.StateProvince;
                if(toUpdatePropertyList.Contains(nameof(AddressDataModel.CountryRegion)))
                    existing.CountryRegion = input.CountryRegion;
                if(toUpdatePropertyList.Contains(nameof(AddressDataModel.PostalCode)))
                    existing.PostalCode = input.PostalCode;
                if(toUpdatePropertyList.Contains(nameof(AddressDataModel.ModifiedDate)))
                    existing.ModifiedDate = input.ModifiedDate;
            }
        }
        ...
        public async Task<Response<AddressDataModel>> Get(AddressIdentifier id)
        {
            ...
            try
            {
                var existing = _dbcontext.Address
                    .SingleOrDefault(t =>
                        id.AddressID.HasValue && t.AddressID == id.AddressID
                );
        ...
        public async Task<Response<AddressDataModel>> Create(AddressDataModel input)
        {
            ...
            try
            {
                ...
                await _dbcontext.SaveChangesAsync();
                return await Get(new AddressIdentifier { AddressID = toInsert.AddressID });
            }
            catch (Exception ex)
            {
                return await Task<Response<AddressDataModel>>.FromResult(new Response<AddressDataModel> { Status = HttpStatusCode.InternalServerError, StatusMessage = ex.Message });
