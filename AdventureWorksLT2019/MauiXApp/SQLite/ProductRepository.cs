...
public class ProductRepository : SQLiteTableRepositoryBase<ProductDataModel, ProductAdvancedQuery, ProductIdentifier>
{
    public ProductRepository(SQLiteService sqLiteService) : base(sqLiteService)
    {
        _database.CreateTable<ProductDataModel>();
    }

    protected override Expression<Func<ProductDataModel, bool>> GetItemExpression(ProductDataModel item)
    {
        return t => t.ProductID == item.ProductID;
    }

    protected override Expression<Func<ProductDataModel, bool>> GetItemExpression(ProductIdentifier identifier)
    {
        return t => t.ProductID == identifier.ProductID;
    }
    ...
    /// <returns></returns>
    protected override Expression<Func<ProductDataModel, bool>> GetSQLiteTableQueryPredicateByAdvancedQuery(ProductAdvancedQuery query)
    {
        return t => true;
