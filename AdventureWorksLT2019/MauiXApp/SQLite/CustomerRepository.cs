...
public class CustomerRepository : SQLiteTableRepositoryBase<CustomerDataModel, CustomerAdvancedQuery, CustomerIdentifier>
{
    public CustomerRepository(SQLiteService sqLiteService) : base(sqLiteService)
    {
        _database.CreateTable<CustomerDataModel>();
    }

    protected override Expression<Func<CustomerDataModel, bool>> GetItemExpression(CustomerDataModel item)
    {
        return t => t.CustomerID == item.CustomerID;
    }

    protected override Expression<Func<CustomerDataModel, bool>> GetItemExpression(CustomerIdentifier identifier)
    {
    ...
    /// <returns></returns>
    protected override Expression<Func<CustomerDataModel, bool>> GetSQLiteTableQueryPredicateByAdvancedQuery(CustomerAdvancedQuery query)
    {
        return t => true;
